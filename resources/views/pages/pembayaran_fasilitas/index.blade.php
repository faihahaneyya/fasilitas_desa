...
@section('content')
    <div class="container-fluid">
        ...
        @endif
        <form method="GET" action="{{ route('pembayaran-fasilitas.index') }}" class="mb-3">
            <div class="row">
                <div class="col-md-2">
                    <select name="metode" class="form-select" onchange="this.form.submit()">
                        <option value="">-- Semua Metode --</option>
                        @foreach(['Transfer Bank', 'Tunai', 'QRIS', 'E-Wallet (OVO/Dana)'] as $m)
                ...
                <div class="col-md-4">
                    <div class="input-group">
                        <input type="text" name="search" class="form-control" value="{{ request('search') }}"
                            placeholder="Cari nama pembayar atau keterangan...">
                        <button type="submit" class="btn btn-primary">Cari</button>
                        @if(request('search') || request('metode'))
                            <a href="{{ route('pembayaran-fasilitas.index') }}" class="btn btn-outline-danger">Reset</a>
                        @endif
                    </div>
                </div>
            </div>
                        ...
                        <div class="card-footer bg-white border-0 pt-0">
                            <div class="d-flex justify-content-end">
                                <div class="btn-group" role="group">
                                    ...
                                    {{-- DELETE --}}
                                    <form action="{{ route('pembayaran-fasilitas.destroy', $item->bayar_id) }}" method="POST"
